...
export const LIMITS = {
  ...
  OUTLINE_TITLE: { min: 1, max: 500 },
  OUTLINE_CONTENT: { max: 100000 },
  OUTLINE_TAGS: { maxArrayLength: 30, maxItemLength: 50 },
  REVIEW_COMMENT: { max: 2000 },
  MISSION_CODE: { max: 200 },
  MISSION_TITLE: { min: 1, max: 500 },
  MISSION_CONTENT: { max: 500000 },
  MISSION_TAGS: { maxArrayLength: 30, maxItemLength: 100 },
} as const;
...
/** 合法的审核状态 */
const VALID_REVIEW_STATUSES: ReadonlySet<string> = new Set(["none", "pending", "approved", "rejected"]);

/** 合法的大纲层级 */
const VALID_OUTLINE_LEVELS: ReadonlySet<string> = new Set([
  "brainstorm", "master", "arc", "chapter",
]);

/** 合法的大纲状态 */
const VALID_OUTLINE_STATUSES: ReadonlySet<string> = new Set([
...
/** 校验大纲层级 */
export function isValidOutlineLevel(level: unknown): level is string {
  return typeof level === "string" && VALID_OUTLINE_LEVELS.has(level);
}

/** 校验大纲状态 */
export function isValidOutlineStatus(status: unknown): status is string {
  return typeof status === "string" && VALID_OUTLINE_STATUSES.has(status);
}

/** 校验审核状态 */
export function isValidReviewStatus(status: unknown): status is string {
  return typeof status === "string" && VALID_REVIEW_STATUSES.has(status);
}
