...
import { sanitizeString, sanitizeBoolean, isValidReviewStatus, LIMITS } from "@/lib/api/validation";
...
export async function PUT(
  ...
  try {
    ...
    if (body.isDraft !== undefined) {
      data.isDraft = sanitizeBoolean(body.isDraft, false);
    }

    if (body.reviewStatus !== undefined) {
      if (!isValidReviewStatus(body.reviewStatus)) return badRequest("无效的审核状态");
      data.reviewStatus = body.reviewStatus;
    }

    if (body.reviewComment !== undefined) {
      data.reviewComment = sanitizeString(body.reviewComment, LIMITS.REVIEW_COMMENT.max) ?? "";
    }
    ...
    // 自动提交版本
    const projMsg = await prisma.project.findUnique({ where: { id: projectId } });
    await autoCommit(projMsg?.name ?? projectId, updated.isPinned ? "置顶消息" : "编辑消息", updated.content.slice(0, 80));
