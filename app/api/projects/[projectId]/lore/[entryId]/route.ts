...
export async function PUT(
  ...
  try {
    ...
    // 自动提交版本
    const projLore = await prisma.project.findUnique({ where: { id: projectId } });
    await autoCommit(projLore?.name ?? projectId, "更新词条", updated.title);
...
export async function DELETE(
  ...
  try {
    ...
    const deletedEntry = await prisma.loreEntry.delete({ where: { id: entryId } });

    // 自动提交版本
    const projLoreDel = await prisma.project.findUnique({ where: { id: projectId } });
    await autoCommit(projLoreDel?.name ?? projectId, "删除词条", deletedEntry.title);
