...
export async function PUT(
  ...
  try {
    ...
    // 自动提交版本
    const projMem = await prisma.project.findUnique({ where: { id: projectId } });
    await autoCommit(projMem?.name ?? projectId, "更新记忆", updated.content.slice(0, 50));
...
export async function DELETE(
  ...
  try {
    ...
    const deletedMem = await prisma.memory.delete({ where: { id: memoryId } });

    // 自动提交版本
    const projMemDel = await prisma.project.findUnique({ where: { id: projectId } });
    await autoCommit(projMemDel?.name ?? projectId, "删除记忆", deletedMem.content.slice(0, 50));
